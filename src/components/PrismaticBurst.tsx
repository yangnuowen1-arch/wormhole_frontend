...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    gl.canvas.style.mixBlendMode = mixBlendMode && String(mixBlendMode) !== 'none' ? mixBlendMode : '';
    ...
    return () => {
      ...
      try {
        (meshRef.current as { remove?: () => void } | null)?.remove?.();
      } catch {
        /* ignore dispose errors */
      }
      try {
        (triRef.current as { remove?: () => void } | null)?.remove?.();
      } catch {
        /* ignore dispose errors */
      }
      try {
        (programRef.current as { remove?: () => void } | null)?.remove?.();
      } catch {
        /* ignore dispose errors */
      }
      try {
  ...
  return <div className="relative w-full h-full overflow-hidden" ref={containerRef} />;
};
